    ...
    <div className="rounded-xl border border-slate-200 dark:border-[#1E3448] bg-white dark:bg-[#162032] p-4 space-y-3">
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="mt-0.5 text-sm font-bold text-slate-900 dark:text-slate-100 leading-snug">{brief.headline}</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-3 gap-2">
        {brief.topSectors.map((s) => (
          <div
            key={s.name}
            className="rounded-lg bg-slate-100 dark:bg-[#0F1923] px-2 py-1.5 text-center"
          >
            <span className={`text-xs font-bold ${DIRECTION_COLOR[s.direction]}`}>
              {DIRECTION_ICON[s.direction]}
            </span>
            <p className="text-xs font-semibold text-slate-700 dark:text-slate-200 mt-0.5">{s.name}</p>
          </div>
        ))}
      </div>
