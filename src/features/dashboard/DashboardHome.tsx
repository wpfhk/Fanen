      ...
      <div className="mx-auto max-w-7xl px-4 py-8 space-y-5">
        ...
        <div className="rounded-xl border border-[#1E3448] overflow-hidden">
          <div className="flex items-center justify-between px-4 pt-3 pb-2">
            <h2 className="text-sm font-semibold text-slate-400">비나 맵 — 오늘의 Hot Zone</h2>
        ...
        <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
          ...
          {/* 포트폴리오 요약 */}
          <div className="rounded-xl border border-[#1E3448] bg-[#162032] p-4">
            <h2 className="text-sm font-semibold text-slate-400">내 포트폴리오</h2>
            <p className="mt-2 text-2xl font-bold text-slate-100">
              {MOCK_PORTFOLIO.totalValue.toLocaleString()}원
            </p>
            <p className={`mt-1 text-sm font-medium ${MOCK_PORTFOLIO.totalReturn >= 0 ? 'text-red-400' : 'text-blue-400'}`}>
              {MOCK_PORTFOLIO.totalReturn >= 0 ? '+' : ''}{MOCK_PORTFOLIO.totalReturn}%
            </p>
            <Link href="/portfolio" className="mt-3 inline-block text-xs text-teal-400 hover:underline">
              상세 보기 →
            </Link>
          </div>
        </div>
        ...
        <div className="rounded-xl border border-[#1E3448] bg-[#162032] p-4">
          <div className="flex items-center justify-between mb-3">
            <h2 className="text-sm font-semibold text-slate-400">오늘의 주요 뉴스</h2>
            <Link href="/news" className="text-xs text-teal-400 hover:underline">전체 보기 →</Link>
          </div>
          <ul className="space-y-2">
            {MOCK_NEWS.map((news) => (
              <li key={news.id} className="flex items-start gap-2 text-sm">
                ...
                <div>
                  <p className="text-slate-200 leading-snug">{news.title}</p>
          ...
          {QUICK_MENU.map((item) => (
            <Link key={item.href} href={item.href} className="group">
              <Card variant="bordered" padding="md" className="text-center transition-shadow group-hover:shadow-md dark:bg-[#162032] dark:border-[#1E3448]">
                ...
                <p className="mt-1 text-sm font-medium text-slate-300 group-hover:text-teal-400 transition-colors">
                  {item.label}
                </p>
              </Card>
            </Link>
