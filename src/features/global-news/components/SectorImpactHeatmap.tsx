        ...
        return (
          <div key={s.sector}>
            <div className="flex items-center justify-between mb-1">
              <span className="text-sm font-medium text-gray-800 dark:text-slate-200">{s.sector}</span>
              <span className={`text-xs font-semibold ${cfg.color}`}>{cfg.label}</span>
            </div>
            <div className="h-2 bg-gray-100 dark:bg-slate-700 rounded-full overflow-hidden">
              <div className={`h-full ${cfg.barColor} ${IMPACT_WIDTH[s.impact]} rounded-full transition-all`}/>
            </div>
            <p className="text-xs text-gray-500 dark:text-slate-400 mt-0.5">{s.reason}</p>
          </div>
        );
      })}
    </div>
